...
Three parameters interact and must be tuned together: `router_init_scale`, `mosrah_overallocation_factor`, and `max_bid_rounds`. `router_init_scale` controls how flat routing logits are at initialization — lower values give the load-balance mechanism earlier leverage by keeping logit magnitude comparable to `expert_bias`, but flatter logits produce more near-tied token competitions at expert capacity boundaries. `mosrah_overallocation_factor` controls how much capacity slack exists above the expected per-head token count — tighter margins mean less room to resolve those competitions. `max_bid_rounds` is the iteration ceiling for the capacity-enforcement bidding algorithm. When logits are flat and overallocation margins are tight, more rounds are needed to reach a stable assignment. If `max_bid_rounds` is too small for the combination of routing flatness and capacity tightness, bidding fails to converge and the model raises an assertion. The default values are chosen to work together; if you tune any one of these parameters significantly, check that the other two remain compatible.
...
2. If tests pass, the architecture is uploaded to the staging Hub repository (`smithblack-0/SHRAM-dev`).
3. E2E network tests run against the staging repository.
4. If E2E tests pass, the architecture is uploaded to the production Hub repository (`smithblack-0/SHRAM`).
