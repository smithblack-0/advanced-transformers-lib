...
\title{A Design Law for Routed Head Sparse Attention: Trading Parameters at at Linear Rate for Long-Sequence Performance}
...
The scaling of long-sequence performance remains one of the central unresolved limitations of modern attention architectures. As attention lengths grow, so too do issues with focus and resolution, due to the underlying issue that the model must handle a longer sequence of tokens with the same set of parameters. \citep{liu2024lost, vasylenko2026longcontextgeneralizationsparseattention}. While existing sparse-attention methods such as LSH, recurrent formulations, and SSMs have enabled computationally operational architectures in the long sequence regime, the ability to scale sequence length easily remains elusive, with most solutions being variations on specially designed architectures or auxiliary memory workarounds. \citep{kitaev2020reformer, katharopoulos2020transformers, gu2022efficiently, piekos2025mosa}.

At a purely \emph{conceptual} level, combining token-routed mixture of expert technology with long-sequence operation has an beneficial property which may enable scaling of long sequence performance by simply scaling hardware. For an expert mixture system which is performing token-choice routing and dividing $N$ tokens up among $L$ experts, the average sequence length per expert is $N/L$. This implies on average attention sequences occur over this length, which is reduced from the full token sequence length by the factor $1/L$ and thus presumably easier to attend to. If we then naively assume that this expert routing does not harm the model when trained, it follows from the marginal sequence length gain per expert -- $\Delta N / L$ -- that a model built on top of these principles should see marginal long sequence performance improve linearly in $L$. Since $L$ is controlled by the number of parameters, this implies such scaling can be done solely in terms of additional hardware.

We find that for the right formally defined architectural class limited theoretical guarantees exist which imply a mixture of attention sparsity will exhibit the hypothesized guarantees. We find furthermore by empirical studies that there [is/is\_not] evidence this effect. We concretely contribute:

\begin{itemize}
    \item The presentation and applicable conditions and limitations of the \emph{Token-Choice Sparse Routed Attention Mixtures} architecture classes, and the corrosponding \emph{SRAM Design Law} which the class enforces.
    \item The empirical study of the \emph{Sparse Hybrid Routed Attention Mixture} (SHRAM) model, a TC--SRAM compliant model designed deliberately to explore the effect, and the contextualization of the result in terms of supporting or negating a viably large beneficial regime as defined under the SRAM design law.
    \item A full derivation of SRAM design law. the model architecture, and how the model architecture satisfies the design law. This is located largely in the appendix for brevity.
\end{itemize}
...
Presume initial embedded input $x \in \mathbb{R}^{B \times N \times d}$ where B is batches, N is sequence length, and d is the embedding width. A \textbf{Token-Choice Sparse Routed Attention Mixture} is an abstract model class characterized by the following invariants.
...
\begin{itemize}
    ...
    \item This formulation is replicated into indexed but individually executed attention suboperators, $A_h(\tilde{x}_h)$, over $L \in \mathbb{Z}$ distinct experts where $0 < h < L-1$. Each replication is independently reinitialized and has independent parameters.
    \item Each input token is routed by token choice into a fixed finite number $K \in \mathbb{Z}$ of selected experts; routing uses additional parameters for each additional possible router, and $0 \geq K\leq L$
...
Satisfaction of all these conditions at a mathematical level by a given concrete implementation is sufficient to classify a layer as a member of the family. It should, however, be emphasized that what is needed for applicability is for a decomposition to exist that mathematically matches this description.\footnote{An interesting consequence is that even Vaswani attention is a form of TC-SRAM in a routing limit. The viewpoint that makes this work is to view attention as a set of independent heads that always start from the embedding, independently project into a bottlenecked head representation $x_h$, attend within it, project back to the main embedding width, then add up the results from each head. This satisfies the invariants, with the heads representing experts.  \citep{vaswani2017attention, wang2023interpretability}.} If there is a total of $N$ tokens to be processed, then by simple arithmetic there are $NK$ tokens after routing to assign and process. It follows by simple arithmetic that the average tokens attended to within a particular expert bucket $\tilde{x}_h$ is given by

\[
\approx \frac{NK}{L}
\]

\subsection{The SRAM Design Law}

An ideally load-balanced TC-SRAM layer is \emph{theoretically bounded to have a regime of operation in which parameters can be traded directly for long-sequence task performance at at least a linear rate}. The formal statement of this effect is as follows.

\begin{quote}
\begin{theorem}[SRAM Design Law]
\label{theorem}

Let $\tau$ denote an arbitrary average long-sequence \emph{task}\footnote{as opposed to computational performance} performance target. Let $K$ be the number of heads selected per token and let $L$ be the total number of heads instantiated by a TC-SRAM attention layer. Let $\Omega$ be the total parameters allocated to those heads, at fixed per-head budget $k$, so that $\Omega=Lk$. Additionally let the TC-SRAM layer have near-perfect load balancing such that the tokens in $\tilde{x}_h$ scale as $\approx \frac{NK}{L}$. Let the additional parameters introduced with increasing $L$ be beneficial for $K \approx L$ and harmful in the extremum $K << L$.
...
A brief semi-formal primer illustrating why the effect takes place is the following

\begin{quote}
If a load-balanced model sparsely divides $N$ tokens $K$ times among $L$ heads, then the tokens per head is on average $\frac{NK}{L}$. Suppose attention is performed on this group; then attention difficulty now decreases with increasing $L$ by our assumptions of attention focus dilution. However, the routing process itself can cause performance changes. When $K \approx L$ we must be able to reproduce the vaswani formulation; therefore, at this location any additional parameters must just be beneficial as they increase capacity. However, at $NK/L < 1$ load balancing assigns at most one token per head which means nothing is attended to at all, an extremely harmful configuration. Therefore, by virtue of starting with a beneficial return and ending with a harmful one, there must be some region $1 > \frac{K}{L} > r$ in which, for whatever definition of 'beneficial' is chosen, a TC-SRAM model with load balancing will see only a beneficial return.
\end{quote}

The fully formal derivation of the theorem is available in appendix \ref{app:proof}.

\subsection{Predictions}

Theorem \ref{theorem} predicts a load-balanced TC-SRAM model has a regime in which additional parameters can be traded for additional long sequence capability at a given performance target.  Long-context benchmarks such as RULER, however, report performance at specified sequence lengths rather than directly returning the maximum supported sequence length at a common score threshold. This makes direct testing difficult.Instead, we may fit a statistical model to predict what the score is at a particular sequence length, then use this model to form predictions all at the same score so the theory can be made directly applicable. This is discussed more deeply in the methodology.

Once this has been done, it becomes possible to use that fit to build a dataset of $(\tau, N, \Omega)$ pairs where $\tau$ is the RULER score targeted, $N$ is the predicted sequence length for that score, and $\Omega$ the total number of parameters invested in MoSRAH layers. A linear fit of theorem \ref{theorem} fit at all scores of $N$ and $\Omega$ may then be performed to discover $d_\tau$ and $b_\tau$. We predict the following

\begin{itemize}
    \item \emph{$d_\tau$ will be positive}: As MoSRAH parameters increase so does supported sequence length at a score
    \item \emph{The fit may be poor}: We have only established a bound, not a direct relationship. It is quite likely the fit will be poor; success of the theorem should be judged on if the slope is positive, not a strong $R^2$ fit.
    \item: \emph{The gains taper off with increasing $\tau$}: as we begin to reach the end of the beneficial region, we begin to slow down.
\end{itemize}

The null hypothesis for this experiment is that no positive slope is found. The alternative hypothesis is that a positive slope is found. A positive slope being found is correlated given our chosen values with a practically large sparsity response regime.

\section{Method}

A specially designed transformer derivative was implemented and pretrained to test this effect.

\subsection{Model}

The model used within this experiment was deliberately conservative. A Llama3 baseline was modified to replace the Grouped Query Attention with a special Sparse Hybrid Routed Attention layer, containing a local attention layer $h_l(x)$ and a MoSRAH layer $h_s$. These were executed as

\[
H(x) = h_l(x) + h_s(x),
\]

Beyond this, the MoSRAH subsystem had it's rope formulation configured to extrapolate using YaRN , and with two positional modes. \citep{peng2024yarn} The "main sequence" mode assigned the position of tokens based on their ordering in the input token sequence $x$. The "semantic sequence" mode assigned rope positions based on index position within the packed group of tokens $x_h$ instead. The latter was hypothesized to allow semantically related but temporally distant tokens to attend more strongly. Besides for changes necessary to support these behaviors, all parts of the model remained standard. The full details may be consulted in appendix \ref{app:architecture}.

\subsection{MoSRAH and SHRAM}

The \emph{Mixture of Sparse Routed Attention Heads} (MoSRAH) layer, $h_s$, was defined specifically to satisfy the restriction conditions applied to TC-SRAM layers, and was the primary component under study in tested \emph{Sparse Hybrid Token Routed Attention Mixture} (SHRAM) model.

Let $x \in \mathbb{R}^{B \times N \times D}$ be the input to the MoSRAH layer. Let $R(x)$ represent the routing process. Let the expert-choice packing map be $E$, the token-choice unpacking map as $E_*^{-1}$, and the parallel attention process as $G$

The MoSRAH forward pass is then given by Algorithm \ref{alg:mosrah}.
...
\State $y \gets G(\tilde{x})$ \Comment{Attention over packed experts (includes rope)}
...
Here $I \in \mathbb{Z}^{B \times N \times K}$ denotes the selected heads per token, $P \in \mathbb{R}^{B \times N \times K}$ the corresponding routing probabilities, $\tilde{x} \in \mathbb{R}^{B \times L \times T \times d}$ the packed expert-choice representation, and $\tilde{y} \in \mathbb{R}^{B \times N \times K \times d}$ the restored token-choice responses prior to reduction.

This architecture was deliberately engineered such that, in the transition from the token-choice form at $x, I$ to the expert-choice form $\tilde{x}$, the expected reduction in average token length $\approx \frac{NK}{L}$ occurs as desired while well load-balanced. All architectural details such as  routing, expert-choice packing, and the Bottlenecked Ensemble Attention (G) formulation are discussed in more detail in the Architecture Appendix, \ref{app:architecture}

\section{Experimental Design}

\subsection{Independent and Dependent Variables}

The MoSRAH layer was pretrained with $K=16$ across total expert values of $L$ at 16, 32, and 64. These corresponded to sparsity ratios of $K/L \in \{1.0, 0.5, 0.25\}$ respectively. Three seeds of value 44, 45, and 46 was tested for each model. The RULER long sequence was then applied at lengths of 4096, 8192, 16,384, and 32,768 to enable the capture of marginal degradation in performance. RULER benchmarks were evaluated in YaRN extrapolative mode. In total, models of [SIZES] were tested.

\subsection{Controlled Variables}

18 models were trained with 12 decoder layers at an embedding width of 512. A vocabulary size of 50277 was utilized to correlate with the chosen GPT-X Neo BPE tokenizer, and unless otherwise noted components of the architecture corrospond to the equivalent variation used in Llama3. Pretraining processed 6000 batches at a batch size of 128 and with a sequence length of 1024, a total of roughly 786M tokens. Pretrainined was executed on all models under torch Automatic Mixed Precision\footnote{This occurred in hybrid float32/bfloat16 format} at isotoken conditions.

Optimization was executed with the AdamW optimizer with the configuration of a learning rate of 6e-4, betas of $(0.9, 0.99)$, and weight decay of $1e-1$. Gradient clipping occurred at a value of 1.0. A cosine annealing learning rate schedule was used with a warmup for 600 batches and which decayed down to 6e-5 by training end. The sliding window attention mechanism used a total of 16 heads with head embedding width of 16 and a window size of 128. Evaluation loss, accuracy, perplexity, wall time delta, and MaxVio were monitored during training to ensure training was successful.

\subsection{Fit Data Construction}

The results of the ruler benchmarks produced information in the form of $\Omega$, the number of parameters in total dedicated to MoSRAH layers, $K$, the number of layers chosen, $N$, the sequence length evaluated at, and as the response variable $S$ the score. Theorem \ref{theorem} however is stated in terms of variation in $N$ and $\Omega$ all at the same score to be properly examined.

To accommodate this we performed a quadratic fit against the RULER data between (N, S) at each $\Omega$, incorporating all seeds. We then reversed this relationship to be able to predict a value of $N$ given a constant value of $S$ at a given $\Omega$, paralleling the assumption of theorem \ref{theorum} of constant $\tau$. We then drew four samples at all values of $\Omega$ and retaining fit accuracies between the observed minimum $S$ and maximum $S$. This formed the data needed to perform the fit analysis to test theorem \ref{theorem}

\subsection{Statistical Analysis}

Once constructed the fits can be plotted for visual inspection. In addition, we quantify the probability that all the slopes are positive into a p-score for statistical thoroughness by fitting theorem \ref{theorem} at each score, judging the hypothesis that the slope is positive, and combining these results together using Fisher's method. We also explored other forms of fits to more exactly match the actual uncovered patterns.
...
In the autoregressive setting studied here, token importance is strongly localized, so a purely sparse routed layer risked prematurely discarding short-range structure. For this reason, attention in the decoder cell was executed as a hybrid cell known as shram containing a local attention component $h_l$ and the MoSRAH sparse attention component $h_s$.
...
This now has exactly $B \times N \times K$ true entries, and active tokens are left-justified; it is in essence the active token mask. However, it is not directly compatible with $\tilde{x}$ and $\tilde{J}$. These two tensors are of shape $(B, N \times K, d)$ and $(B, N \times K)$ respectively. However, masked transfers must be done with 1d indexing regions. As such a small additional reshape is necessary.\footnote{This can safely be executed as a view for memory efficiency.}

\[
\tilde{J}' = \mathrm{Reshape}(\tilde{J}) \in \mathbb{Z}^{B* N * K},
\qquad
...
The following metrics were collected throughout pretraining. All metric payloads always included the wall time delta from the last clock step, the current token count, the current optimizer step, and the current batch number when being logged in addition to whatever is specifically logged.

\textbf{Training Metrics}

These metrics were taken every step during training

\begin{itemize}


    \item \texttt{raw train loss}: ($\mathcal{L}_\mathrm{CE}$) The raw cross-entropy loss on the current training batch,
    computed for causal next-token prediction in a teacher-forced configuration.
    \item \texttt{raw load balancing loss}: ($\mathcal{L}_\mathrm{lb}$) The raw auxiliary load-balancing loss produced by
    the MoSRAH routing layer, prior to application of the load-balance weight.
    \item \texttt{effective loss}: ($\mathcal{L}_\mathrm{eff}$) The weighted combined loss that backpropagation
    operated on, defined as $\mathcal{L}_\mathrm{eff} = \lambda_\mathrm{s} \cdot \mathcal{L}_\mathrm{CE}
        + \lambda_\mathrm{lb} \cdot \mathcal{L}_\mathrm{lb}$
    \item \texttt{gradient norm}: The pre-clip gradient norm. Also used to activate anomaly reocvery if needed.
    \item \texttt{effective learning late}: The product of the scheduled learning rate
    and the instability scale $\lambda_\mathrm{s}$, representing the true learning rate
    the model responded to. Anomaly recovery could adjust this
    \item \texttt{effective load balancing weight}: The current value of
    $\lambda_\mathrm{lb}$ after any anomaly recovery doublings, representing the true
    load-balance penalty weight in effect at that step. Anomaly recovery could adjust this.
\end{itemize}

\subsubsection{Evaluation Metrics}

\begin{itemize}
    \item \textbf{Evaluation Loss}: The raw cross-entropy loss on the validation split.

    \item \textbf{Load Balance Loss}: The raw auxiliary load-balancing loss on the
    validation split, recorded to assess whether routing balance generalizes to
    held-out data.

    \item \textbf{Perplexity}: $\exp(\mathcal{L}_\mathrm{eval})$, derived from the
    raw evaluation loss.

    \item \textbf{Accuracy}: Fraction of correct next-token predictions on non-padding
    positions in the validation split.

    \item \textbf{Wall Time Delta}: Elapsed wall time since the previous evaluation,
    used to monitor training throughput.

    \item \textbf{MaxVio}: The maximum routing imbalance scalar, defined as
    $L \cdot \max_l(f_l - 1/L)$, where a value of zero indicates perfect load balance.
    See Appendix \ref{app:maxvio} for a full definition.
\end{itemize}
