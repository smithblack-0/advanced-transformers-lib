"""Produce a Hub-compatible staging directory from the mosa model source directory.

The mosa model directory is already flat — no subdirectories contain Python files,
...
def stage(source_dir: Path, dest_dir: Path) -> None:
    ...
    The mosa source tree is flat; every file is copied directly to dest_dir root
    ...
    Args:
        source_dir: Source model directory (e.g. src/mosa/model/).
