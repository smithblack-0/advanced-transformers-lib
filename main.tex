...
The empirical goal of this study was to probe whether the tradeoff predicted by theorem \ref{theorem} appears in practice under controlled conditions. To do so, only the total routed head capacity of the sparse routed path was varied across the main comparison runs, while the remainder of the model and training configuration was held fixed unless otherwise noted. Concretely, the MoSRAH path always selected $K=16$ heads per token, while the total number of routed heads was varied across $L \in \{16, 32, 64\}$, corresponding respectively to sparsity ratios $K/L \in \{1.0, 0.5, 0.25\}$. Architectures of [Sizes] were examined.

\subsubsection{Standards}

Research standards was executed in a theory-first, contract-first manner. The architectural design, theoretical framing, and implementation-relevant details of the apparatus were specified prior to experimental execution as an incomplete paper\footnote{Technically, this paper was written first, then the results were substituted into it when they became available. This is a benefit of theory-first ML}. The system was then implemented against this specification, with LLM assistance used in a contract-realization role rather than as an autonomous design authority with heavy human oversight.

All research and coding was executed to a professional demonstrative standard suitable for reproduction and examination by third parties. Research specified exact hyperparameters and the ways to resolve uncertainties before the LLM build; this is required for LLM contract-first development to be successful. No research or coding was allowed to proceed while any ambiguity was detected; such events were viewed as a hard blocker that had to be documented and resolved by team consensus before the development trajectory could proceed. An exact log of the entire development trajectory as a living document was maintained in the 'plan.md' file in the codebase at all times; this later served as the foundation for the final audit.
...
\]
Where $\Omega$ designates parameters assigned to MoSRAH layers in total. Evidence for the predicted tradeoff was sought in regions exhibiting positive slope within the first quadrant, with either sign of curvature permitted. Evidence against the predicted tradeoff would be detected if no such region of increasing slope could be detected on the fit.
...
The SHRAM layer was loaded into a Llama3 model. The sliding window attention mechanism used a total of 16 heads with head embedding width of 16 and a window size of 128. The MoSRAH layers also chose $K=16$ heads at head embedding width of 16. The total number of MoSRAH heads were tested at amount of $L$ = 16, 32, and 64 for sparsity ratios of 1.0, 0.5, and 0.25 respectively.\footnote{This configuration was chosen to divide compute roughly equally between the local attention formulation and the long-range MoSRAH layers}

All models were trained at an embedding width of 512, with 12 total decoder layers. A vocabulary size of 50277 was utilized to correlate with the chosen GPT-X Neo BPE tokenizer. A prenorm architecture with RMSNorm was used between all major layers in the style of Llama3. SwiGLU layers were executed at hidden widths of 1366. \footnote{This is the standard $\mathrm{embedding\_width}*\frac{8/3}$ applied at 512} Dropout was not used, and all long-sequence extrapolation was done using RoPE formulations operating in YaRN extrapolation mode. Details on hyperparameter tuning  -- or the lack of it -- are available in appendix \ref{app:hyperparameter_tuning}

\subsection{Pretraining}

Pretrainined was executed on all models under torch Automatic Mixed Precision\footnote{This occurred in hybrid float32/bfloat16 format} at isotoken conditions. [Num] models were pretrained on the FineWeb-Edu dataset at the seeds of 44, 45, and 46. (\citep{penedo2024fineweb}) A total of $\approx 655\mathrm{M}$ tokens were processed over the course of 5000 batches for each at a batch size of 128. \footnote{This exceeds the Kaplan recommendations applicable to the small-sized control model. (\citep{kaplan2020scaling})}. Due to the small model size no epochs needed to be repeated.

Opimization was executed with AdamW optimizer at a learning rate of 6e-4 and betas of $\beta_1=0.9$ and $\beta_2 = 0.99$. Weight decay was set to a strength of $1e-1$. Gradient clipping occurred at a value of 1.0. A cosine annealing schedule from 6e-4 to 6e-5 was used with a warmup period of 500 batches. The main loss was cross-entropy configured for causal next-token-prediction in a teacher-forced configuration. The auxiliary loss for load balancing was weighed by 1e-2 before addition to the main loss following which backpropagation occurred. (\citep{fedus2021switch})

Checkpoints were taken at every 1000 batches. Evaluation loss, accuracy, perplexity, wall time delta, and the MaxVio\footnote{Consult appendix \ref{app:maxvio} for more details on this definition, or alternatively the original source at \citet{wang2024auxiliarylossfree}} load balancing metric was taken every 200 batches. Due to the limited computational resource\footnote{The pretraining budget was 100\$} available an an anomaly recovery strategy \footnote{Consult appendix \ref{app:anomaly_recovery} for a clear definition} was utilized to not waste any pretraining run, even if it might degrade the data. This proved [necessary/not\_necessary]. Training occurred on [Hardware] for [Time].

\subsubsection{Processes and Auditing}
